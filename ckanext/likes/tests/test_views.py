...
def test_page(app: CKANTestApp):
    ...
    resp = app.get(tk.url_for("likes.page"))
    assert resp.status_code == 200
    assert resp.body == "Hello, likes!"
...
def test_page_redirect(app: CKANTestApp):
    ...
    resp = app.get(
        tk.url_for("likes.page_redirect"),
    ...
    expected = tk.url_for("likes.page", _external=True)
...
class TestComplex:
    def test_get(self, app: CKANTestApp, faker: Faker):
        ...
        word = faker.word()
        resp = app.get(tk.url_for("likes.complex", word=word))
    ...
    def test_post_invalid(self, app: CKANTestApp, faker: Faker):
        ...
        word = faker.word()
        resp = app.post(tk.url_for("likes.complex", word=word))
        page = bs4.BeautifulSoup(resp.body)
    ...
    def test_post_valid(self, app: CKANTestApp, faker: Faker):
        ...
        word = faker.word()
        resp = app.post(
            tk.url_for("likes.complex", word=word),
            data={"left": "1", "right": "2"},
        )
        page = bs4.BeautifulSoup(resp.body)
