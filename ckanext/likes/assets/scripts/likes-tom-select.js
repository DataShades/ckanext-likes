 ...
 */
ckan.module("likes-tom-select", function () {
  ...
  return {
    ...
    // `this.sandbox.likes.nestedOptions` and we receive
    // `{helloWorld: {byeWorld: ...}}`.
    options: {
    ...
    initialize() {
      ...
      if (typeof TomSelect === "undefined") {
        ...
        console.error("[likes-tom-select] TomSelect library is not loaded");
        return
      }

      // tom-select has a number of nested options. We are using
      // `nestedOptions` helper defined inside `likes.js` to
      // convert flat options of CKAN JS module into nested object.
      const options = this.sandbox["likes"].nestedOptions(this.options);
