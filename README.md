...
Code of the extension resides inside `ckanext/likes`.
...
| Blanket             | Effect                                                                                              |
|---------------------|-----------------------------------------------------------------------------------------------------|
| actions             | Register all public functions from `ckanext.likes.logic.action` as actions        |
| auth_functions      | Register all public functions from `ckanext.likes.logic.auth` as auth functions   |
| blueprints          | Register all blueprints from `ckanext.likes.views` as blueprints                  |
| cli                 | Register all public members(`__all__`) from `ckanext.likes.cli` as commands       |
| config_declarations | Register all declarations from `ckanext/likes/config_declaration.yaml`            |
| helpers             | Register all public functions from `ckanext.likes.helpers` as helpers             |
| validators          | Register all public functions from `ckanext.likes.logic.validators` as validators |
...
function defined inside `ckanext.likes.logic.action` will be
...
class LikesPlugin(p.SingletonPlugin):
    ...
```
...
First, create a module inside `ckanext.likes.implementations`
...
Re-export implementation from `ckanext/likes/implementations/__init__.py`
...
the module. As result, only this group will be available as `ckan likes` CLI command.
...
```sh
ckan config declaration likes -d
```
...
* config options can be accessed by shorter name: `option()` instead of
  `tk.config["ckanext.likes.option.name"]`.
...
`assets/webassets.yml` and include this named asset into template using `{% asset "likes/ASSET_NAME" %}` tag.
...
template for the blog page may be stored as `templates/likes/blog/index.html`.
...
to generate migrations for the model using `ckan generate migration -p likes -m "Migration message"` CLI command.
...
suggest leaving them inside `ckanext/likes/schemas`.
...
`ckanext/likes/tests` contains examples of tests for standard
...
`ckanext/likes/tests/benchmarks` contains benchmarks. They
...
pytest --cov=ckanext.likes

## generate HTML report at htmlcov/index.html
pytest --cov=ckanext.likes --cov-report html
```
...
Run tests from `ckanext/likes/tests/test_plugin.py`

```sh
pytest ckanext/likes/tests/test_plugin.py
```
...
contains filepath, class and test name: `ckanext/likes/tests/test_smth.py:TestSmth:test_smth`
